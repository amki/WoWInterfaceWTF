
ConfirmSP = {
	["profileKeys"] = {
		["Thankubliz - Tarren Mill"] = "Default",
		["Intergalacto - Azshara"] = "Default",
		["Ieni - Tarren Mill"] = "Default",
		["Intergalacto - Onyxia"] = "Default",
		["Rynllor - Tarren Mill"] = "Default",
		["Enthusiastus - Tarren Mill"] = "Default",
		["Mihri - Tarren Mill"] = "Default",
		["Iridia - Tarren Mill"] = "Default",
		["Intergalacto - Tarren Mill"] = "Default",
		["Enthusiastus - Azshara"] = "Default",
	},
	["profiles"] = {
		["Default"] = {
			["version"] = "02.16 2018-12-16",
			["popups"] = {
				["CONFIRM_REMOVE_FRIEND"] = false,
				["BATTLE_PET_PUT_IN_CAGE"] = true,
				["CONFIRM_DELETE_TRANSMOG_OUTFIT"] = false,
				["CONFIRM_BINDER"] = true,
				["CONFIRM_BUY_REAGENTBANK_TAB"] = false,
				["TALENTS_INVOLUNTARILY_RESET"] = false,
				["CONFIRM_BUY_BANK_SLOT"] = false,
				["CONFIRM_RESET_TO_DEFAULT_KEYBINDINGS"] = false,
				["GOSSIP_CONFIRM"] = false,
			},
		},
	},
}
